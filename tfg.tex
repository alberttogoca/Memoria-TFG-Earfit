...
Parece ser la más completa y elaborada de todas con más de 2500 ejercicios (ver Figura \ref{fig:earmaster}). Contiene ejercicios para que puedas reconocer, transcribir, leer, cantar y tocar: Intervalos, acordes, progresiones armónicas, escalas, ritmos y melodías.
\cite{earmaster1}
...
Este trabajo se centra en el siguiente proceso combinando de Design Thinking, Lean Startup y Metodologías Ágiles para la creación de un producto mínimo viable. De esta manera conseguimos un proceso que parte de la nada, se centra en el usuario, gestiona el desarrollo, integra el error y es iterable.
\cite{estructura}
...
En la Figura \ref{fig:LeanDesignAgile} se ilustra este proceso que se ha dividido en la memoria en los siguientes tres capítulos:
...
El proceso de Design Thinking se compone de cinco etapas y no es lineal. Por tanto, en cualquier momento se puede saltar entre etapas si se cree oportuno. De modo que a lo largo del proceso se va afinando el contenido hasta desembocar en una solución que cumpla con los objetivos. 
\cite{desingThinking2}


\begin{figure}[H] 
    \includegraphics[width=\textwidth]{Design Thinking/Etapas}
    \centering
    \caption{Etapas de Design Thinking.}
    \label{fig:DesignThinking}
\end{figure}

En la Figura \ref{fig:DesignThinking} podemos observar las etapas de las que se compone Design Thinking. El objetivo que pretende abodar cada etapa es el siguiente:
\begin{itemize}
...
Para idear una solución que aporte valor, realizamos un \textbf{MindMap} \cite{mindmap}. Esta es una herramienta gráfica para representar ideas o conceptos y encontrar soluciones.
En ella tratamos de enfocarnos en los principales problemas del usuario y sus posibles soluciones.
...
En la Figura \ref{fig:Mindmap} se observa este Mindmap donde se concluye que una posible forma de solucionar estos problemas es mediante una aplicación web con ejercicios de entrenamiento auditivo. 

\subsection{Prototipar}
\label{sec:prototipar}

En este etapa utilizamos la técnica \textbf{MoSCoW} \cite{moscow}. Esta es una técnica de priorización utilizada en gestión de proyectos.
En ella tratamos de establecer las prioridades del proyecto, teniendo en cuenta nuestras limitaciones: poco tiempo para desarrollar, aprender conceptos musicales y nuevas tecnologías. 
...
En la Figura \ref{fig:MoSCoW} se observa esta técnica de MoSCoW. Esta técnica se basa en definir lo que debe tener (Must have), debería incluir (Should have), podría incluir (Could have) y no va a tener (Won't have) la aplicación.

Para crear nuestro \textbf{producto mínimo viable} se decidió centrarse en desarrollar los Must haves y más tarde los Should haves. Los Could haves y los Won't haves se deciden dejar para trabajos futuros por complejidad y tiempo de desarrollo.

Por último, una vez establecidas las prioridades del proyecto pasamos a visualizarlas diseñando el prototipo teniendo en cuenta la filosofía de diseño \textbf{Atomic Design} \cite{atomicdesign} y \textbf{Mobile First} \cite{mobilefirst}. Diseñando primero para pantallas pequeñas y después adaptándolo a pantallas más grandes. 
...
Para no extender este trabajo se puede encontrar una explicación detallada de como fue este proceso de crear, medir, aprender en el apéndice \hyperref[sec:LeanStartup]{\textbf{A.3 Lean Startup}}. A continuación, nos centraremos sólo en la fase de creación del software.
\cite{leanstartup2}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Scrum/Scrum}
    \caption{Marco de trabajo Scrum.}
    \label{fig:Scrum}
\end{figure}

%El \textbf{flujo de trabajo} en Scrum como se ilustra en la Figura \ref{fig:Scrum} es el siguiente:
En la la Figura \ref{fig:Scrum} podemos observar el \textbf{flujo de trabajo} de Scrum. Este flujo de trabajo está formado por los siguientes pasos:
...
Implementar este marco de trabajo nos permite obtener resultados pronto, reduciendo el \textbf{Time to Market}, es decir, a tener lo antes posible en el mercado nuestro producto o una de sus características, aumentando la satisfacción del cliente. 
Además, permite lidiar con requisitos cambiantes o poco definidos, aportando tolerancia al cambio.
...
\href{https://miro.com/app/board/uXjVOMmPLjM=/?share_link_id=588058562303}{\texttt{https://miro.com/app/board/uXjVOMmPLjM=/}}
...
En nuestro caso usamos \textbf{Github} \cite{github}, que es un servicio basado en la nube, cómo herramienta de control de versiones \textbf{Git} \cite{git}. Y utilizamos \textbf{Gitflow} \cite{gitflow}, que es un modelo alternativo de creación de ramas en Git.
...
En la Figura \ref{fig:Gitflow} podemos observar este modelo de creación de ramas. Este se caracteriza por el uso de varias ramas de función, una rama de desarrollo y una rama de producción:
...
Para ello utilizamos \textbf{GitHub Actions} \cite{githubActions} el cuál nos permite personalizar y automatizar estos despliegues desde nuestro repositorio. Y para alojar nuestra aplicación web usamos \textbf{Vercel} \cite{vercelNext}, que es una plataforma en la nube optimizada para proyectos \textbf{Nextjs}.
...
En la Figura \ref{fig:vercel_workflow} podemos observar el flujo de trabajo. 
Este se basa en que cada vez que se realiza una subida de código, Github Actions hace un despliegue en Vercel, siguiendo el \textbf{flujo de trabajo DPS} \cite{dps}: 

\begin{itemize}
    \item \textbf{Desarrollo}: Escribimos código en Next.js y subimos los cambios a una rama en Github.
    \item \textbf{Vista previa}: Vercel crea un despliegue de cada rama que estará disponible a través de una URL. Podemos compartir esta URL para recibir feedback. 
...
Creado por Vercel, es un framework creado sobre \textbf{Node.js} y basado en \textbf{React}.
...
Por último, ofrece \textbf{Vercel Analytics} \cite{vercelAnalytics} para medir el rendimiento de la web, recopilando las web vitals de los dispositivos reales que utilizan los usuarios.
...
Los navegadores no entienden TypeScript, por lo que es necesario transpilarlo a JavaScript antes de usarlo \cite{typescript2}. Al crear nuestra aplicación con \textbf{Nextjs} ya obtenemos transpilación y empaquetado automáticos con \textbf{SWC} y \textbf{Webpack}.
...
Debe estar instalado con \textbf{NPM} \cite{npm} o \textbf{Yarn} \cite{yarn}, dos gestores de paquetes para proyectos de Node.js. A veces, la \textbf{instalación de paquetes} con NPM no es lo suficiente consistente o rápida, dando incluso errores. Este es el motivo para usar Yarn, una alternativa construida por Facebook, Google, Exponent y Tilde \cite{npmyarn}.
...
En la Figura \ref{fig:Arquitectura} se ilustra la arquitectura de la aplicación. Ésta está formada por \textbf{varias capas desacopladas}: infraestructura, datos, servicios e interfaz de usuario. Estas capas interactúan entre sí de la siguiente manera:

\begin{itemize}
    \item \textbf{Infraestructura}: Se compone de funciones, librerías y tipos de Typescript que apoyan la lógica de los servicios y de la interfaz.
    \item \textbf{Servicios}: Proveen los datos y alguna lógica independiente a la capa de interfaz de usuario.
    \item \textbf{Contexto y Hooks}: Usan los servicios y se hacen cargo de crear y manejar los estados necesarios para los componentes.
    \item \textbf{Páginas y Componentes}: Hacen uso del contexto y los Hooks, y renderizan la interfaz de usuario.
\end{itemize}
...
\textbf{Next.js} tiene algunos archivos y \textbf{directorios especiales} \cite{nextjs2}. Como mínimo, se necesita una carpeta ``pages'' con un archivo ``index.js''. Además, la carpeta ``public'' tiene una función específica en Next.js. Aparte de estos archivos y directorios, todo vale \cite{reactArchivos}.
...
En la Figura \ref{fig:Archivos} se puede observar la estructura de archivos. En este caso se han separado las \textbf{carpetas por conceptos} o intereses. 
Cada carpeta contiene archivos similares basandose en la arquitectura:
...
\textbf{Archivos de configuración}: En el directorio raíz del proyecto se pueden encontrar los archivos ``.eslintrc.js'', ``.gitignore'', ``.prettier.js'', ``next-env.d.ts'', ``next.config.js'', ``tsconfig.json'', ``package.json'' y ``yarn.lock''. 
También se encuentra un archivo ``README.md'' con una breve presentación y explicación de la aplicación.
...
%En Next.js, una página es un componente de React exportado desde un archivo .js, .jsx, .ts o .tsx en el directorio de páginas. Cada página está asociada con una ruta basada en su nombre de archivo.
%Nextjs prerenderiza cada página usando Static Generation.
%Cada HTML generado está asociado con el código JavaScript mínimo necesario para esa página. 
%Cuando el navegador carga una página, su código JavaScript se ejecuta y hace que la página sea completamente interactiva. (Este proceso se llama hidratación.)
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{Detalles de Implementación/Code/Pages/Intervals2}
    \caption{Código de la página de intervalos.}
    \label{fig:IntervalsPage}
\end{figure}

%En la Figura \ref{fig:IntervalsPage} se observa el código de una de las páginas de la aplicación. Su lógica es similar para todas las demás:
En la Figura \ref{fig:IntervalsPage} se observa el código de una de las páginas de la aplicación. Explicando su lógica se puede entender el comportamiento de todas las demás:
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{Detalles de Implementación/Code/Components/AnswerButtons2}
    \caption{Código del componente AnswerButtons.}
    \label{fig:AnswerButtons}
\end{figure}

En la Figura \ref{fig:IntervalsPage} se observa el código de uno de los componentes de la aplicación. Explicando su lógica se puede entender el comportamiento de todos los demás:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{Detalles de Implementación/PWA/InstalarPWA}
...
Los principios de Clean Code \cite{cleancode} permiten obtener código limpio, reutilizable, escalable y con mayor cambiabilidad. Algunos de los principios adaptados React tenidos presentes se encuentran en el apéndice \hyperref[sec:cleancode]{\textbf{E. Clean Code}}.
...
%Las pruebas funcionales que se han realizado se clasifican en las siguientes: \hyperref[sec:unitarias]{\textbf{Pruebas Unitarias}}, \hyperref[sec:components]{\textbf{Pruebas de Componentes}}, \hyperref[sec:integracion]{\textbf{Pruebas de Integración}} y \hyperref[sec:e2e]{\textbf{Pruebas End to End (E2E)}}.
%Cada una pretende abordar un objetivo en concreto donde se han intentado abordar todos los test cases o casos de prueba posibles.

Cada prueba que se ha realizado pretende abordar un objetivo en concreto donde se han intentado abordar todos los test cases o casos de prueba posibles. Estas pruebas se clasifican en: 
\hyperref[sec:unitarias]{\textbf{Pruebas Unitarias}}, \hyperref[sec:components]{\textbf{Pruebas de Componentes}}, \hyperref[sec:integracion]{\textbf{Pruebas de Integración}} y \hyperref[sec:e2e]{\textbf{Pruebas End to End (E2E)}}.
...
En este caso según se iba desarrollando el código se comprobaba que cada Hook, función y estado, devolvía el resultado esperado y funcionaba correctamente antes de darle uso en el código. 
...
De esta manera en cada momento se tiene una visión general del estado del proyecto alertándonos de regresiones en el código.
...
Es una herramienta para medir la calidad de las páginas web. %Audita el rendimiento, la accesibilidad, y la optimización de páginas web. También incluye la capacidad de probar aplicaciones web progresivas para el cumplimiento de estándares y mejores prácticas.
Incluye la capacidad de probar PWA para el cumplimiento de estándares, mejores prácticas. Permite detectar errores y oportunidades de optimización. 
%Los resultados de su análisis se encuentran en la Figura \ref{fig:Lighthouse_resumen} y consta de las siguientes categorías:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Lighthouse/Resumen}
    \caption{Puntuaciones de Lighthouse.}
    \label{fig:Lighthouse_resumen}
\end{figure}

En la Figura \ref{fig:Lighthouse_resumen} se observan los resultados de su análisis de la aplicación. Estos constan de las siguientes cinco categorías:
...
\begin{itemize}
    ...
    \item \textbf{Best Practices}: En este área se analizan los aspectos de seguridad de la web. Se comprueban las tecnologías de codificación como TLS, si los recursos, bibliotecas JavaScript, bases de datos y APIs son seguras.
    ...
    \item \textbf{Progressive Web Apps}: Esta es la función principal de Google Lighthouse. Analiza si la página web funciona según lo previsto. Comprueba si todos los elementos y contenidos dinámicos se representan correctamente, si la página registra un Service Worker y si está disponible la función offline.
\end{itemize}
...
En la Figura \ref{fig:Vercel_metrics} se observa la vista de análisis de la aplicación. Esta se calcula en base a las \textbf{Web Vitals}, que son una colección de métricas establecidas por Google \cite{webdev} y el Web Performance Working Group \cite{w3}. 
En ella pueden distinguir las siguientes cuatro categorías:
...
El presente trabajo se proponía el desarrollo de una herramienta que permitiera a los músicos desarrollar su oído musical mediante la práctica de ejercicios de \hyperref[sec:eartraining]{\textbf{entrenamiento auditivo}}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.35\textwidth]{Conclusiones/EarfitQR}
...
Esta memoria recoge todo lo necesario para seguir con el trabajo aún si no se conocen las tecnologías y metodologías empleadas. 
Por lo que, se podrá considerar mejorar la aplicación o implementar algunas de las funcionalidades que no tuvieron cabida en el desarrollo.

Algunas de las \textbf{posibles extensiones} para la aplicación, que no se llevaron a cabo por falta de tiempo son:
...
\begin{itemize}
    ...
    \item \textbf{Software libre vs Negocio}. De momento esta aplicación es gratuita. Se podría plantear alguna forma de monetización, si se considerase que la aplicación pudiera tener algún hueco en el mercado, aunque este no era su propósito original.
\end{itemize}
